...
export const revalidate = 86400; // Seite für 24 Stunden im Vercel CDN cachen
        ...
        <div>
          {example.secondAfter ? (
            <div className="flex flex-col gap-6">
              <Image src={example.after} alt={example.title} width={1200} height={800} className="w-full rounded-2xl shadow-2xl border border-white/10 opacity-90 hover:opacity-100 transition-opacity duration-700 mx-auto cursor-pointer" />
              <Image src={example.secondAfter} alt={example.title + ' Detail'} width={1200} height={800} className="w-full rounded-2xl shadow-2xl border border-white/10 opacity-90 hover:opacity-100 transition-opacity duration-700 mx-auto cursor-pointer" />
            </div>
          ) : example.after && example.before ? (
            <BeforeAfter beforeSrc={example.before} afterSrc={example.after} alt={example.title} />
          ) : example.after ? (
            <Image src={example.after} alt={example.title} width={1200} height={800} className="w-full rounded-2xl shadow-2xl border border-white/10 opacity-90 hover:opacity-100 transition-opacity duration-700 mx-auto" />
          ) : (
